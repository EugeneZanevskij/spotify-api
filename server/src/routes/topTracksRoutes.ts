...
router.post('/short-term', async (req: Request, res: Response) => {
  try {
...
router.get('/medium-term', async (req: Request, res: Response) => {
  const { userId } = req.body;
  try {
...
router.post('/medium-term', async (req: Request, res: Response) => {
  try {
    const { userId, tracks } = req.body;
    const user = await prisma.user.findUnique({
      where: { id: userId },
