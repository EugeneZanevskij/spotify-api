...
const RecentlyPlayedTrackItem : React.FC<{recentlyPlayedTrack: RecentlyPlayedTrack}> = ( {recentlyPlayedTrack} ) => {
  const trackDuration = () => {
  ...
  return (
    <TrackItem>
      <TrackImage src={recentlyPlayedTrack.track.album.images[0].url} alt={recentlyPlayedTrack.track.album.name} />
      <TrackContainer gap='0'>
        <BoldText fontSize='1.2em'>{recentlyPlayedTrack.track.name}</BoldText>
        <Text>{recentlyPlayedTrack.track.artists.map((artist) => artist.name).join(', ')}</Text>
      </TrackContainer>
      <TrackContainer>
